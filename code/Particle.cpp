...
Particle::Particle(RenderTarget& target, int numPoints, Vector2i mouseClickPosition) : m_A(2, numPoints)
{
    ...
    if (rand() % 2)  m_vx *= -1;
    if (rand() % 2)  m_vy *= -1; 
    sf::Color RandColor1(rand() % 250, rand() % 250, rand() % 250);
...
void Particle::draw(RenderTarget& target, RenderStates states) const 
{
    ...
    Vector2f center = target.mapPixelToCoords(target.mapCoordsToPixel(m_centerCoordinate, m_cartesianPlane));
    lines[0].position = center;
    lines[0].color = m_color1;
    for (int j = 1; j <= m_numPoints; j++)
    {
        lines[j].position = target.mapPixelToCoords(target.mapCoordsToPixel({ (float)m_A(0, j - 1), (float)m_A(1,j-1) }, m_cartesianPlane));
